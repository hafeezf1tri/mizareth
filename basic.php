...
<html lang="en">
  <head>
    ...
    <title>Ultracafe Cyber Device Booking System || Basic PCs</title>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="product-bit-title text-center">
                        <h2 style="font-family: Roboto Condensed">Basic PCs</h2>
                    </div>
                </div>
            </div>
        </div>
...
//basic default page num
        if (isset($_GET['pageno'])) {
            $pageno = $_GET['pageno'];
        } else {
            $pageno = 1;
        }
        // paging formula showcase
        $no_of_records_per_page = 4;
        $offset = ($pageno-1) * $no_of_records_per_page;
        $total_pg_sql = "SELECT COUNT(*) FROM tbldevice";
        $ret1=mysqli_query($con,"select COUNT(*) from  tbldevice where Type='Basic'");

        $total_rows = mysqli_fetch_array($ret1)[0];
        $total_pg = ceil($total_rows / $no_of_records_per_page);                                                   
 $query=mysqli_query($con,"select * from tbldevice where Type='Basic' LIMIT $offset, $no_of_records_per_page");
